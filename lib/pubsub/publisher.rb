  ...
  class Publisher
    ...
    def prepare_attributes(options)
      Pubsub.reloader.call do
        concrete_opts = Object.const_get(options['job_class']).pubsub_options || {}
        base_opts = Object.const_get(options['class']).pubsub_worker_options || {}
      ...
      @topic = if @options.delete('morgue')
                 Pubsub.topics['morgue']
               else
                 Pubsub.topics[@options['queue_name'].to_s]
               end
    end
  end
end
